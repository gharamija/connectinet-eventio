...
function Header(props) {
    ...
    const pagesPiO = [{label: "Moji događaji", onclick: getMojiDogadaji}, {
        label: "Svi događaji",
        onclick: getDogadaji
    }];
    const pagesAdmin = [{label: "Svi korisnici", onclick: getKorisnici}, {label: "Svi događaji", onclick: getDogadaji}];
    const settings = [{label: "Profil", onClick: getProfil}, {label: "Odjava", onClick: logout}];
    ...
    return (
        <AppBar position="static"
                sx={{
                    marginBottom: '20px',
                    backgroundColor: props.role === "Posjetitelj" ? '#2196F3' :   // Blue for Posjetitelj
                        props.role === "Organizator" ? '#4CAF50' : // Green for Organizator
                            props.role === "Admin" ? '#FF0000' :       // Red for Admin
                                '#000000'
                }}>
                    ...
                    <Box sx={{flexGrow: 0}}>
                        ...
                        >
                            {settings.map((setting) => (
                                <MenuItem key={setting.label} onClick={setting.onClick || handleCloseUserMenu}>
